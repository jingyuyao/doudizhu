...
class PlaySpec extends FlatSpec {
  ...
  "Three three" should "be a triplet" in {
    Play.maybeCreate(all("3").take(3)) match {
  ...
  "Two twos" should "be a pair" in {
    Play.maybeCreate(all("2").take(2)) match {
  ...
  "One ace" should "be a single" in {
    Play.maybeCreate(all("A").take(1)) match {
  ...
  "3 to 7" should "be a sequence" in {
    Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+")) match {
  ...
  "Random numbers" should "not be a play" in {
    Play.maybeCreate(all("3+", "5+", "7+", "8+", "10+")) match {
      case Some(_) => fail()
      case _ => succeed
    }
  }
  ...
  "4 to 8" should "beat 3 to 7" in {
    val threeToSeven = Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+")).get
    val fourToEight = Play.maybeCreate(all("4+", "5+", "6+", "7+", "8+")).get
    assert(fourToEight.canBeat(threeToSeven))
    assert(!threeToSeven.canBeat(fourToEight))
  }

  "3 to 8" should "not beat 3 to 7" in {
    val threeToEight = Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+", "8+")).get
    val threeToSeven = Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+")).get
  ...
  "Rocket" should "beat everything" in {
    val rocket = Play.maybeCreate(jokers).get
    val single = Play.maybeCreate(all("A").take(1)).get
    val double = Play.maybeCreate(all("A").take(2)).get
    val triplet = Play.maybeCreate(all("A").take(3)).get
    val bomb = Play.maybeCreate(all("A").take(4)).get
    val sequence = Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+")).get
  ...
  "Bomb" should "beat everything except for rocket" in {
    ...
    val triplet = Play.maybeCreate(all("A").take(3)).get
    val bomb = Play.maybeCreate(all("A").take(4)).get
    val bomb2 = Play.maybeCreate(all("2").take(4)).get
    val sequence = Play.maybeCreate(all("3+", "4+", "5+", "6+", "7+")).get
